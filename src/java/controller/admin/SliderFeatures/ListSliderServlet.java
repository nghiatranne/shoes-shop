 ...
 * @author USA
 */
@WebServlet(name="ListSliderServlet", urlPatterns={"/admin/sliders"})
    ...
    throws ServletException, IOException {
      String query = request.getParameter("q");
        ...
        if (query != null && !query.trim().isEmpty()) {
            sliders = sliderDAO.searchSlidersByTitle(query.trim());
        } else {
            sliders = sliderDAO.getAllSliders();
        }
        request.setAttribute("sliders", sliders);
        request.getRequestDispatcher("/views/admin/ListSlider.jsp").forward(request, response);
    } 
